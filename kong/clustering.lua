...
function _M.init_worker(conf)
  ...
  elseif conf.role == "control_plane" then
    ...
    -- Handles "clustering:invalidation" cluster event
    local function handle_clustering_invalidation_event(data)
      ngx_log(ngx_DEBUG, "received clustering:invalidation event for ", data)
      post_push_config_event()
    end


    -- Handles "dao:crud" worker event and broadcasts "clustering:invalidation" cluster event
    local function handle_dao_crud_event(data)
      if type(data) ~= "table" or data.schema == nil or data.schema.db_export == false then
        return
      end

      kong.cluster_events:broadcast("clustering:invalidation", data.schema.name .. ":" .. data.operation)
    ...
    -- The "clustering:invalidation" cluster event gets inserted in the cluster when there's
    ...
    -- its node
    kong.cluster_events:subscribe("clustering:invalidation", handle_clustering_invalidation_event)

    -- The "dao:crud" event is triggered using post_local, which eventually generates an
    -- ""clustering:invalidation" cluster event. It is assumed that the workers in the
